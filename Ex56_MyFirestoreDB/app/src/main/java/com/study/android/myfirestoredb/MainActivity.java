package com.study.android.myfirestoredb;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void  onBtn1Clickled(View v){
       dataInsert();
    }
    public void  onBtn2Clickled(View v){
        String sDelete= etNum.getText().toString();
        dataDelete(sDelete);
    }
    public  void dataQuery(){
        //서버에서 일단가져온 후에 서버에서 데이터가 변경되면
        //폰의 화면에서 실시간으로 자도 갱신 된다
        int LIMIT=30;
        db.collection("MyFirestoreDB")
    //             .orderBy("name",Query.Drection.DESCENDING)
    //             .limit(LIMIT)
        ...
        public void dataDelete(String sDelete) {
            ...
            db.collection("MyFirestoreDB")
                    ...
                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {
                            Log.d(TAG,"DocumentSnashot successfully delete!");
                        }
                    })
                    .addOnFailureListener(new OnFailureListener() {
                        @Override
